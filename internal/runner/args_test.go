...
// There are no tend-overlay tests here any more. runner.Effective used to
// resolve a tend: section over the loop's agent:, because a loop hosted the
// project's tends and ran them with a different agent from its own. The tend
// dispatcher has its own configuration now, whose agent: IS the tend agent, so
// what those tests pinned -- which model, harness and effort a tend runs on --
// belongs to config.LoadTend and is asserted there.
//
// The one property that survives here is the label override, which still wins
